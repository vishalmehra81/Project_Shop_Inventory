...
def save(product):
    sql = "INSERT INTO products (name, brand_id, stock_id, category, size, cost_price, selling_price) VALUES (%s,%s," \
...
def select(id):
    ...
    product = Product(result["name"], brand, stock, result["category"]), result["size"], result["cost_price"], \
...
def update(product):
    sql = "UPDATE products SET (name, brand_id, stock_id, category, size, cost_price, selling_price) " \
          "VALUES (%s,%s,%s,%s,%s,%s,%s) WHERE id = %s"
    values = [product.name, product.brand.id, product.stock.id, product.category, product.size, product.cost_price,
